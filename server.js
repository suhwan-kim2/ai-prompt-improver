...
**절대적 규칙 (위반시 실패):**
1. 사용자가 제공한 정보만 정확히 반영하세요
2. 사용자가 선택하지 않은 옵션은 절대 추가하지 마세요
3. 질문에 없던 내용은 절대 임의로 만들지 마세요
4. 사용자 답변에 없는 기술적 세부사항 금지
5. 사용자 답변에 없는 도구/프로그램 이름 금지
6. 사용자 답변에 없는 수// server.js - Express 서버 (v5.0 - 일반/전문가 모드 + 의도 파악 시스템)
...
전문가모드 개선 전략 (최고 품질 요구):
- ${rounds}회차 질문을 통해 수집된 모든 심층 정보 활용
- 사용자의 숨겨진 의도와 요청사항 정확히 반영
- 업계 최고 수준의 세밀한 요구사항 포함
- 국제 표준과 베스트 프랙티스 적용

전문가급 필수 포함 요소:
1. **명확한 역할 정의**: "당신은 [분야] 전문가입니다"
2. **상세한 기술 명세**: 구체적 수치, 해상도, 형식, 품질 기준
3. **단계별 실행 가이드**: 체계적이고 논리적인 제작 순서
4. **품질 관리 기준**: 성공 측정 방법과 평가 기준
5. **전문 용어 활용**: 해당 분야의 정확한 기술 용어
6. **예외 상황 대응**: 제약조건과 대안 방안
7. **최종 결과물 명세**: 구체적 포맷과 전달 방식
8. **성과 측정 방법**: 객관적 평가 기준

분야별 전문가급 개선:
- 개발: 아키텍처 패턴, 성능 최적화, 보안 프로토콜, 테스트 전략
- 영상제작: 촬영 기법, 편집 워크플로우, 색감 보정, 음향 설계
- 글쓰기: 구조적 접근, 독자 분석, 스타일 가이드, SEO 최적화
- 디자인: 디자인 시스템, 브랜드 가이드라인, 사용성 테스트

품질 기준 (전문가급):
- 모든 문장이 구체적이고 실행 가능해야 함
- "좋게", "적당히" 같은 모호한 표현 절대 금지
- 수치와 기준이 명확히 제시되어야 함
- 단계별 체크포인트가 포함되어야 함
...
일반모드 필수 요소:
1. **명확한 지시사항**: 애매함 없는 구체적 요청
2. **기본 품질 기준**: 해상도, 길이, 형식 등 핵심 명세
3. **실행 단계**: 간단한 제작 순서
4. **결과물 형태**: 원하는 최종 결과 명시
...
1. 사용자가 명시한 모든 요구사항을 정확히 포함
2. 사용자가 선택한 옵션들을 구체적으로 반영
3. 요청사항에 기재된 세부 의도를 반영
4. 명시되지 않은 부분은 일반적 가이드라인으로만 처리
5. 전문성은 구조와 논리로 보여주되 임의 추가 금지
