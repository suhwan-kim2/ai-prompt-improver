...
function updateSelectedCategories() {
    ...
    const selectedText = selectedCategories.map(cat => categoryNames[cat]).join(', ');
...
// AI 답변 처리
async function processAIAnswers() {
...
function displayQualityResult(qualityData, original, improved) {
    ...
    // 점수에 따른 색상 설정
    let scoreColor = '#28a745'; // 초록색 (7점 이상)
    if (qualityData.score < 7) scoreColor = '#ffc107'; // 노란색 (5-7점)
    if (qualityData.score < 5) scoreColor = '#dc3545'; // 빨간색 (5점 미만)
    ...
    const qualityHTML = `
        <div class="quality-section" style="margin-top: 20px; padding: 20px; background: #f8f9fa; border-radius: 15px; border: 2px solid ${scoreColor};">
            <h4 style="color: #333; text-align: center; margin-bottom: 15px;">
                🎯 AI 품질 평가: <span style="color: ${scoreColor}; font-size: 24px;">${qualityData.score}/10점</span>
            </h4>
    ...
    // 완전 자동화 결정 (테스트를 위해 임계값 조정)
    if (qualityData.score < 9) {
        // 9점 미만이면 자동으로 완전 자동화 시스템 시작
        showAutoFullSystem(original, improved, qualityData);
    } else {
        showStatus(`완벽한 품질입니다! (${qualityData.score}/10점)`, 'success');
        setTimeout(function() {
            askSatisfaction();
        }, 500);
    }
...
function showAutoFullSystem(original, improved, qualityData) {
    ...
    const autoSystemHTML = `
        <div class="auto-system-section" style="text-align: center; margin-top: 15px; padding: 20px; background: linear-gradient(135deg, #667eea, #764ba2); border-radius: 15px; color: white;">
            ...
            <p style="margin-bottom: 15px; opacity: 0.9;">현재 ${qualityData.score}점입니다. AI가 9점 이상까지 자동으로 개선하겠습니다!</p>
...
async function startFullAutoImprovement() {
    ...
    try {
        ...
        showStatus('🤖 AI가 완전 자동 모드로 9점급 프롬프트를 만들고 있습니다...', 'processing');
        ...
        while (attempts < maxAttempts) {
            ...
            showStatus(`🎯 ${attempts}회차 완료: ${newQuality.score}/10점 달성`, 'processing');
            
            // 9점 이상 달성시 종료
            if (newQuality.score >= 9) {
                showStatus(`🎉 ${attempts}회차만에 9점 달성! 완전 자동 개선 완료!`, 'success');
            ...
            if (attempts >= maxAttempts) {
                showStatus(`⚡ ${maxAttempts}회 개선 완료: 최종 ${newQuality.score}/10점`, 'success');
                document.getElementById('improvedText').textContent = currentImproved;
                await showFinalQualityResult(original, currentImproved, newQuality, attempts);
            }
        }
...
async function startQuickAutoImprovement() {
    ...
    try {
        ...
        showStatus(`🎉 빠른 자동 개선 완료: ${finalQuality.score}/10점 달성!`, 'success');
...
async function quickQualityCheck(original, improved) {
    try {
        ...
        return { score: 7, feedback: '품질 확인 완료' };
    }
}
    ...
    const finalHTML = `
        <div class="final-quality-section" style="text-align: center; margin-top: 15px; padding: 20px; background: linear-gradient(135deg, #28a745, #20c997); border-radius: 15px; color: white;">
            <h4 style="color: white; margin-bottom: 15px;">
                🏆 자동 개선 완료: ${qualityData.score}/10점 (${attempts}회차)
            </h4>
...
function proceedWithCurrent() {
    ...
    if (autoSection && autoSection.innerHTML.includes('자동으로 9점급')) {
        autoSection.remove();
    }
