...
(function (global) {
  ...
  const SIGNAL_RULES = [
    ...
    {
      /* 좌석번호를 5nn·800번대처럼 흐리게 적는 것. 정확히 적으면 예매처가
       * 대조해 예매를 취소시킬 수 있어 일부러 가리는 경우가 많다. */
      re: /(\d\s*[nN]{1,2}\s*번?대?|\d00\s*번대|[nN]{1,2}\s*열|[nN]{2,}번)/,
      requires: /(구역|스탠딩|지정석|열|번대|좌석)/,
      ground: '좌석번호를 일부러 흐리게 표기해 좌석이 특정되지 않도록 하고 있음',
      label: '좌석번호 가림'
    },
    {
