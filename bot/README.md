...
좌석번호와 예매번호가 둘 다 없으면 경고한다. 좌석번호가 `5nn`, `800번대`,
`n열` 처럼 **가려져 있을 때도 경고한다** — 신고센터는 좌석이 특정되어야
유효 접수로 처리하는데, 판매자들은 예매처가 대조해 취소시키지 못하도록
일부러 흐리게 적는다. 이런 건은 판매자에게 예매번호를 받거나, 플랫폼
자체 신고로 게시글을 내리는 쪽이 빠르다.

### ③ file

**자동화 브라우저를 띄우지 않는다.** 한국 본인인증(PASS 등)은 자동화된
브라우저를 막는 경우가 많아 그 창에서는 인증을 끝낼 수 없다. 대신
사용자의 평소 브라우저로 넘긴다.

1. 폼 데이터를 클립보드에 복사한다
2. 신고서 페이지를 기본 브라우저로 연다
3. 북마크 바의 「신고서 채우기」를 누르면 항목이 채워진다 (초록 테두리)
4. 본인인증 · 증빙첨부 · 제출은 그 브라우저에서 사용자가 한다

북마클릿은 처음 한 번만 설치하면 된다. `file` 을 실행하면
`bookmarklet.html` 을 만들어주니, 열어서 버튼을 북마크 바로 드래그한다.
클립보드 복사가 막히면 데이터를 화면에 그대로 보여주므로 직접 복사해도 된다.

예매처는 감시 목록의 `ticketUrl` 주소에서 정한다(멜론티켓·인터파크·YES24·
티켓링크·쿠팡플레이·기타). 판매자 닉네임은 판매글 페이지에서 직접 읽는다 —
번개장터는 `@핸들` 형태가 아니라 본문 파서로는 잡히지 않는다.
링크주소 칸에는 `http`로 시작하는 주소일 때만 넣는다.
