...
class IntentAnalyzer {
  ...
  // 슬롯 충족 여부 판단(콜론/언더스코어/공백 표기 모두 허용)
  isFilled(key, mentions, text) {
    if (mentions[key]?.length) return true;
    const escape = (s) => s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const k1 = key;                    // e.g., audio_caption
    const k2 = key.replace(/_/g, " "); // e.g., audio caption
    const patterns = [
  ...
  defaultWeights(domain) {
    const map = {
      image: {
        subject: 22, style: 20, ratio_size: 18, lighting_camera: 18, use_rights: 12, negatives: 10
      },
      video: {
        purpose: 22, length: 18, style: 18, platform: 14, audio_caption: 14, rights: 14
      },
      dev: {
        type: 20, core_features: 22, target_users: 16, tech_pref_constraints: 18, priority: 12, security_auth: 12
      }
    };
    return map[domain] || {};
  }
